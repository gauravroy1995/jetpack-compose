...
@Composable
fun VegetableScreen(viewModel: VegetableViewModel,onNextClick: () -> Unit,onCancelClick: () -> Unit){
    ...
    Column {
        LazyColumn(contentPadding = PaddingValues( horizontal = 10.dp, vertical = 20.dp), modifier = Modifier.weight(1f) ){
            items(items=DataSource.vegetables, itemContent = { item ->
                ...
                EachRow(id=item.id, title = item.name, price = item.price, benefits = item.benefits, selected = item.id == selectedVegetable, onClick = { id ->
                    ...
                    viewModel.setVegetableId(id)
                    viewModel.setVegetablePrice(item.price)
                },
        ...
        RowButtons(onFirstClick =  onCancelClick , onSecondClick =  onNextClick , modifier = Modifier
            .fillMaxWidth()
            .padding(16.dp)
            .zIndex(2f) )
